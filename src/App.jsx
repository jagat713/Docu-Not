...
const editNote = (index) => {
  ...
  return (
    ...
    <div className='grid grid-cols-1 md:grid-cols-2 h-screen w-full items-center gap-2 grid-wrap'>
      <div className='text-white bg-gray-950 w-full  rounded-2xl border-2 h-full flex flex-col p-4 gap-4 border-gray-700'>
        <form action="submit" className="flex flex-col gap-3 h-full">
          ...
          <textarea placeholder="Hi..." value={content} onChange={Content} className=" h-[90%] border-none outline-none ring-0 focus:outline-none focus:ring-0 resize-none text-left text-white"></textarea>
      ...
      <div className='text-white bg-gray-950 w-full h-full border-2 rounded-2xl border-gray-700 p-4 flex flex-col gap-4'>
        <h1 className='text-white text-center italic font-bold'>Your Notes</h1>
        <div className=' bg-gray-950 grid grid-cols-1 gap-1 p-2  overflow-y-scroll'>
          {note.map((n, index) => (
